...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uISO
...
void ThreadScriptCheck() {
    RenameThread("ecurrency-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1400547745, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x336b59c1 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x33, 0x6b, 0x59, 0xc1 };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
	...
	if (((pfrom->nVersion < 70002) && ((pindexBest->nHeight) > 29978)))
	{
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EcurrencyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EcurrencyMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("EcurrencyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ecurrency-miner");
    ...
    try { loop {
        ...
        printf("Running EcurrencyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EcurrencyMiner terminated\n");
        throw;
    }
}
